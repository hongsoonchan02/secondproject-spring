...
public class DashboardController {
    ...
    //대시보드 조회
    //GET /api /dashboard/{emplyeeId}
    //@PathVariable -> URL의 {employeeId} 값을 가져오는 것
    ...
    //출근처리
    //POST /api/dashboard/{employeeId}/checkin
    @PostMapping("/{employeeId}/checkin")
    @Operation(summary = "출근 처리", description = "출근 버튼 클릭 시 출근 처리. 이미 출근한 경우 에러 반환")
    public ResponseEntity<CheckInResDTO> checkIn(@PathVariable Long employeeId) {
        return ResponseEntity.ok(dashboardService.checkIn(employeeId));
    }

    //퇴근처리
    // PUT /api/dashboard/{employeeId}/checkout
    // PUT -> 기존 데이터를 수정할 때 사용(출근기록에 퇴근시각 업데이트)
    @PutMapping("/{employeeId}/checkout")
    @Operation(summary = "퇴근 처리", description = "퇴근 버튼 클릭 시 퇴근 시각 및 근무시간 기록")
